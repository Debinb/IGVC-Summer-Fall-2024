...
def hough_transform(image):
    ...
    threshold = 80       # Only lines that are greater than threshold will be returned. Higher the number, fewer lines detected
...
def send_command_to_esp32(action):
    url = f"http://<192.168.4.1/{action}"
    try:
        requests.get(url)
    except:
